...
public class APITest {
	...
	public static void setup() {
		RestAssured.baseURI = "http://localhost:8088/tasks-backend";
	}
	...
	public void deveAdicionarTarefaComSucesso() {
		RestAssured.given().body("{\"task\": \"Teste via RestAssured\",\"dueDate\": \"2022-06-01\"}")
				.contentType(ContentType.JSON).when().post("/todo").then().statusCode(201);
	}

	@Test
	public void naoDeveAdicionarTarefaInvalida() {
		RestAssured.given().body("{\"task\": \"Teste via RestAssured\",\"dueDate\": \"2010-05-29\"}")
	...
	public void deveRemoverTarefaComSucesso() {
		...
		Integer id = RestAssured.given()
				.body("{\"task\": \"Teste via RestAssured para remoção\",\"dueDate\": \"2022-06-01\"}")
